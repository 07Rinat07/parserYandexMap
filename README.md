...
Средний рейтинг, число оценок и число отзывов извлекаются отдельно из текста и rating-блоков карточки. Счетчики поддерживают обычный формат и сокращения вроде `1,2 тыс.`. Селекторы и UI-признаки extraction layer вынесены в `parser/extraction.js`, чтобы адаптировать parser при изменениях DOM без переписывания orchestration-кода. Parser возвращает строго JSON в stdout, технические сообщения пишет в stderr.
...
- Browser pool и очереди внутри parser microservice.
- Более глубокая нормализация относительных дат отзывов.
- График динамики рейтинга в dashboard.
- Alerting в Slack/Telegram при росте parser failures.
- Автоматический production deploy после CI.
